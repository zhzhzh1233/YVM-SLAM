...
public:
    ...
    // External dynamic mask/detections (YOLO-World ROS bridge).
    void SetExternalMask(const cv::Mat &mask);
    void SetExternalDetections(const std::map<std::string, std::vector<cv::Rect2i>> &detections);
    void SetActivePrompts(const std::vector<std::string> &prompts);
...
private:
    ...
    ObjectOptimizer* mpObjectOptimizer;
