...
namespace ORB_SLAM3
{
    ...
    Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp, string filename)
    {
        ...
        if (mpDetector)
        {
            ...
            mLastDetections = mpDetector->mmDetectMap;
        ...
        mLastDepth = imDepth.clone();
        mLastRGB = imRGB.clone();
        ...
        if (gLogORB.load(std::memory_order_relaxed))
            std::cout << "[ORB] Features: " << mCurrentFrame.N << std::endl;
        ...
        UpdateMapObjectsFromDetections();
        if (mpMapDrawer)
            mpMapDrawer->SetCurrentFrameRGBD(imRGB, imDepth, mCurrentFrame.GetPose());
    ...
    bool Tracking::ComputeMedianDepth(const cv::Mat &depth, const cv::Rect2i &bbox, float &medianDepth) const
    {
        if (depth.empty() || depth.type() != CV_32F)
            return false;

        const int x0 = std::max(0, bbox.x + static_cast<int>(bbox.width * 0.3f));
        const int y0 = std::max(0, bbox.y + static_cast<int>(bbox.height * 0.3f));
        const int x1 = std::min(depth.cols, bbox.x + static_cast<int>(bbox.width * 0.7f));
        const int y1 = std::min(depth.rows, bbox.y + static_cast<int>(bbox.height * 0.7f));

        if (x1 <= x0 || y1 <= y0)
            return false;

        const float maxDepth = (mThDepth > 0.0f) ? mThDepth : 10.0f;
        std::vector<float> values;
        values.reserve((x1 - x0) * (y1 - y0));
        ...
        medianDepth = values[mid];
        return true;
    }

    void Tracking::UpdateMapObjectsFromDetections()
    {
        if (mState != OK || mLastDetections.empty() || mLastDepth.empty())
            return;
        ...
        static const std::unordered_set<std::string> kStaticLabels = {
            "chair", "tv", "keyboard", "laptop", "monitor", "bench", "sofa", "table"
        };

        Sophus::SE3f Twc = mCurrentFrame.GetPose().inverse();
        const float cx = Frame::cx;
        const float cy = Frame::cy;
        const float invfx = Frame::invfx;
        const float invfy = Frame::invfy;

        const float kMatchDist = 1.0f;
        for (const auto &entry : mLastDetections)
        {
            const std::string &label = entry.first;
            if (kStaticLabels.find(label) == kStaticLabels.end())
                continue;

            for (const auto &bbox : entry.second)
            {
                float depth = 0.0f;
                if (!ComputeMedianDepth(mLastDepth, bbox, depth))
                    continue;

                const float u = bbox.x + 0.5f * bbox.width;
                const float v = bbox.y + 0.5f * bbox.height;
                const float x = (u - cx) * depth * invfx;
                const float y = (v - cy) * depth * invfy;
                Eigen::Vector3f Pc(x, y, depth);
                Eigen::Vector3f Pw = Twc * Pc;
                cv::Point3f posWorld(Pw.x(), Pw.y(), Pw.z());

                MapObject *matched = nullptr;
                const std::vector<MapObject*> objects = pMap->GetAllMapObjects();
                for (MapObject *obj : objects)
                {
                    if (!obj || obj->GetLabel() != label)
                        continue;
                    const cv::Point3f prev = obj->GetPosWorld();
                    const float dx = prev.x - posWorld.x;
                    const float dy = prev.y - posWorld.y;
                    const float dz = prev.z - posWorld.z;
                ...
                if (matched)
                {
                    matched->AddObservation(posWorld);
                    matched->SetLastBBox(bbox);
                    if (gLogObj.load(std::memory_order_relaxed))
                        std::cout << "[Obj] Update " << label << " id=" << matched->Id()
                                  << " pos=(" << posWorld.x << "," << posWorld.y << "," << posWorld.z << ")"
                                  << " obs=" << matched->GetObservationCount() << std::endl;
                    if (!matched->IsVLMRequested())
                        RequestVLM(matched);
                }
                else
                {
                    MapObject *obj = new MapObject(label, -1, posWorld);
                    obj->AddObservation(posWorld);
                    obj->SetLastBBox(bbox);
                    pMap->AddMapObject(obj);
                    if (gLogObj.load(std::memory_order_relaxed))
                        std::cout << "[Obj] New " << label << " id=" << obj->Id()
                                  << " pos=(" << posWorld.x << "," << posWorld.y << "," << posWorld.z << ")" << std::endl;
                    RequestVLM(obj);
                }
            }
        }
    ...
    bool Tracking::ParseSizeFromJson(const std::string &body, cv::Point3f &sizeOut) const
    {
        size_t keyPos = body.find("\"size\"");
        if (keyPos == std::string::npos)
            return false;
        size_t lb = body.find('[', keyPos);
        ...
        if (!(ss >> z))
            return false;
        sizeOut = cv::Point3f(x, y, z);
        return true;
    }

    std::string Tracking::MatToBase64(const cv::Mat &img) const
    {
        ...
        {
            const float scale = std::min(static_cast<float>(maxDim) / img.cols,
                                         static_cast<float>(maxDim) / img.rows);
            cv::resize(img, toEncode, cv::Size(), scale, scale, cv::INTER_AREA);
        }

        std::vector<uchar> buf;
        std::vector<int> params = {cv::IMWRITE_JPEG_QUALITY, 80};
        if (!cv::imencode(".jpg", toEncode, buf, params))
            return std::string();
    ...
    void Tracking::RequestVLM(MapObject* obj)
    {
        if (!obj || obj->IsVLMRequested())
            return;

        obj->SetVLMRequested(true);
        const std::string label = obj->GetLabel();

        const cv::Rect2i bbox = obj->GetLastBBox();
        const cv::Mat rgb = mLastRGB.clone();
        if (rgb.empty())
        {
            obj->SetVLMRequested(false);
            return;
        }

        cv::Rect roi = bbox & cv::Rect(0, 0, rgb.cols, rgb.rows);
        if (roi.area() <= 0)
        {
            obj->SetVLMRequested(false);
            return;
        ...
        const cv::Mat crop = rgb(roi).clone();
        const std::string img_base64 = MatToBase64(crop);
        if (img_base64.empty())
        {
            obj->SetVLMRequested(false);
            return;
        ...
        if (gLogVLM.load(std::memory_order_relaxed))
            std::cout << "[VLM] Send label=" << label
                      << " id=" << obj->Id()
                      << " roi=" << roi.width << "x" << roi.height
                      << " b64=" << img_base64.size() << std::endl;

        std::thread([this, obj, label, img_base64]() {
            const char *kVLMHost = "http://192.168.134.111:5000";
            httplib::Client cli(kVLMHost);
            cli.set_read_timeout(10, 0);
            cli.set_connection_timeout(5, 0);
            const std::string body = std::string("{\"label\":\"") + label + "\",\"image\":\"" + img_base64 + "\"}";
            auto res = cli.Post("/predict_size", body, "application/json");
            if (res && res->status == 200)
            {
                cv::Point3f size;
                if (ParseSizeFromJson(res->body, size))
                {
                    obj->SetSize(size);
                    if (gLogVLM.load(std::memory_order_relaxed))
                        std::cout << "[VLM] Reply label=" << label
                                  << " id=" << obj->Id()
                ...
                else
                {
                    obj->SetVLMRequested(false);
                    if (gLogVLM.load(std::memory_order_relaxed))
                        std::cout << "[VLM] Parse failed label=" << label
                                  << " id=" << obj->Id()
                                  << " body=" << res->body << std::endl;
                }
            }
            else
            {
                obj->SetVLMRequested(false);
                if (gLogVLM.load(std::memory_order_relaxed))
                    std::cout << "[VLM] Request failed label=" << label
                              << " id=" << obj->Id()
