...
void Viewer::SetActivePrompts(const std::vector<std::string> &prompts)
{
    std::lock_guard<std::mutex> lock(mMutexPrompts);
    mActivePrompts = prompts;
}
...
void Viewer::Run()
{
    ...
    pangolin::CreateWindowAndBind("YVM-SLAM: Map Viewer",1024,768);
    ...
    pangolin::Var<bool> menuShowCurrentCloud("menu.Show Current Cloud",false,true);
    pangolin::Var<bool> menuShowTrajectory("menu.Show Trajectory",true,true);
    pangolin::Var<bool> menuLogObj("menu.Log Obj",true,true);
    pangolin::Var<bool> menuLogVLM("menu.Log VLM",true,true);
    pangolin::Var<bool> menuLogGTSAM("menu.Log GTSAM",true,true);
    pangolin::Var<bool> menuLogORB("menu.Log ORB",true,true);
    ...
    Ow.SetIdentity();
    cv::namedWindow("YVM-SLAM: Current Frame");
    ...
    while(1)
    {
        ...
        gLogObj.store(menuLogObj, std::memory_order_relaxed);
        gLogVLM.store(menuLogVLM, std::memory_order_relaxed);
        gLogGTSAM.store(menuLogGTSAM, std::memory_order_relaxed);
        gLogORB.store(menuLogORB, std::memory_order_relaxed);
        ...
        mpMapDrawer->SetDrawCurrentCloud(menuShowCurrentCloud);
        if(menuShowCurrentCloud)
            mpMapDrawer->DrawCurrentCameraCloud();
        ...
        // Yolo detections (2D boxes + labels)
        {
        ...
        if (!mask.empty())
        {
            ...
            if (mask3Channel.size() == toShow.size() && mask3Channel.type() == toShow.type())
                cv::addWeighted(toShow, 0.7, mask3Channel, 0.3, 0, toShow);
        }
        cv::imshow("YVM-SLAM: Current Frame",toShow);
