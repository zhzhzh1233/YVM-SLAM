	...
	try
	{
		...
		else if (isCuda && (cuda_available != available_providers.end()))
		{
			...
			if (status != nullptr) {
				...
				std::cerr << "Failed to append CUDA EP: " << api->GetErrorMessage(status) << std::endl;
		...
		// Avoid thread affinity issues on Jetson by using a small, explicit thread pool.
		_OrtSessionOptions.SetIntraOpNumThreads(1);
		_OrtSessionOptions.SetInterOpNumThreads(1);
		//
		...
		if (isCuda && warmUp) {
			...
			std::cerr << "ReadModel exception: " << e.what() << std::endl;
			return false;
		}
		catch (...) {
			std::cerr << "ReadModel unknown exception." << std::endl;
			return false;
		}
	return true;
}
