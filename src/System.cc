...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
    ...
    mptObjectOptimizer(static_cast<std::thread*>(NULL)), mpObjectOptimizer(nullptr)
{
    ...
    //Initialize the Object Optimizer thread and launch
    mpObjectOptimizer = new ObjectOptimizer(mpAtlas);
    mptObjectOptimizer = new thread(&ObjectOptimizer::Run, mpObjectOptimizer);
...
void System::Shutdown()
{
    ...
    if (mpObjectOptimizer)
    {
        mpObjectOptimizer->RequestStop();
        if (mptObjectOptimizer && mptObjectOptimizer->joinable())
            mptObjectOptimizer->join();
    }
    /*if(mpViewer)
    {
...
void System::SetExternalMask(const cv::Mat &mask)
{
    if (mpDetector)
        mpDetector->SetExternalMask(mask);
}

void System::SetExternalDetections(const std::map<std::string, std::vector<cv::Rect2i>> &detections)
{
    if (mpDetector)
        mpDetector->SetExternalDetections(detections);
}

void System::SetActivePrompts(const std::vector<std::string> &prompts)
{
    if (mpViewer)
        mpViewer->SetActivePrompts(prompts);
}
