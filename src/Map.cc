...
void Map::EraseMapObject(MapObject *pObj)
{
    unique_lock<mutex> lock(mMutexMap);
    auto it = std::find(mvpMapObjects.begin(), mvpMapObjects.end(), pObj);
    if (it != mvpMapObjects.end())
