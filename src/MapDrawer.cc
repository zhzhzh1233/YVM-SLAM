...
void MapDrawer::SetCurrentFrameRGBD(const cv::Mat &rgb, const cv::Mat &depth, const Sophus::SE3f &Tcw)
{
    std::unique_lock<std::mutex> lock(mMutexRGBD);
    mCurrentRGB = rgb.clone();
    mCurrentDepth = depth.clone();
    mCurrentTcw = Tcw;
    mbHasCurrentRGBD = true;
}

void MapDrawer::SetDrawCurrentCloud(bool enable)
{
    std::unique_lock<std::mutex> lock(mMutexRGBD);
    mbDrawCurrentCloud = enable;
}

void MapDrawer::DrawCurrentCameraCloud()
{
    cv::Mat rgb;
    cv::Mat depth;
    Sophus::SE3f Tcw;
    {
        std::unique_lock<std::mutex> lock(mMutexRGBD);
        if (!mbHasCurrentRGBD || !mbDrawCurrentCloud)
            return;
        rgb = mCurrentRGB;
        depth = mCurrentDepth;
        Tcw = mCurrentTcw;
    ...
    if (rgb.empty() || depth.empty() || depth.type() != CV_32F)
        return;
    ...
    for (int v = 0; v < depth.rows; v += step)
    {
        ...
        for (int u = 0; u < depth.cols; u += step)
        {
            ...
            const float x = (u - cx) * d / fx;
            const float y = (v - cy) * d / fy;
            Eigen::Vector3f Pc(x, y, d);
            Eigen::Vector3f Pw = Twc * Pc;

            const cv::Vec3b color = rgb.at<cv::Vec3b>(v, u);
            glColor3f(color[2] / 255.0f, color[1] / 255.0f, color[0] / 255.0f);
...
void MapDrawer::DrawMapObjects()
{
    ...
    glLineWidth(mKeyFrameLineWidth * 2.0f);
    ...
    for(MapObject* obj : objects)
    {
        ...
        const cv::Point3f c = obj->GetPosWorld();
        const cv::Point3f s = obj->GetSize();
        const float hx = 0.5f * s.x;
        const float hy = 0.5f * s.y;
        const float hz = 0.5f * s.z;

        const float x0 = c.x - hx;
        const float x1 = c.x + hx;
        const float y0 = c.y - hy;
        const float y1 = c.y + hy;
        const float z0 = c.z - hz;
        const float z1 = c.z + hz;
...
void MapDrawer::DrawTrajectory()
{
    ...
    std::vector<KeyFrame*> vpKFs = pActiveMap->GetAllKeyFrames();
    if(vpKFs.size() < 2)
        return;
