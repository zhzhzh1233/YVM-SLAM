...
bool TemplatedVocabulary<TDescriptor,F>::loadFromTextFile(const std::string &filename)
{
    ...
    while(f >> pid >> nIsLeaf)
    {
        if(pid < 0 || pid >= static_cast<int>(m_nodes.size()))
        {
            std::cerr << "Vocabulary loading failure: invalid parent id" << endl;
            return false;
        }

        TDescriptor desc;
        if constexpr (std::is_same<TDescriptor, cv::Mat>::value)
        {
            desc.create(1, F::L, CV_8U);
            unsigned char *p = desc.template ptr<unsigned char>();
            for(int iD=0;iD<F::L;iD++)
            {
                int n = 0;
                if(!(f >> n))
                {
                    std::cerr << "Vocabulary loading failure: invalid descriptor" << endl;
                    return false;
                }
                p[iD] = static_cast<unsigned char>(n);
            }
        }
        else
        {
            ...
            F::fromString(desc, ssd.str());
        }

        double weight = 0.0;
        if(!(f >> weight))
        {
            std::cerr << "Vocabulary loading failure: invalid weight" << endl;
            return false;
        }
